        ...
        if (winer != null) {
         ...
         // 记录信息
    		if (winer.getPlayerId() > 0) {
    		    ...
    		    PlayerInfoCenterManager.getInstance().addPlayerInfoCenter(winer.getPlayerId(), false, Constants.WIN_INFO, "",msg );
    		}
...
//                        log.info("TaskAndRecord|begin|" + (System.currentTimeMillis() - now));
                        ...
                        for (Gamer joinGamer : list) {
                            ...
                            if (joinGamer.getPlayerId() == winer.getPlayerId()) {
                                om.putByte((byte) 1);
                                om.putInt(table.getTotalStake() - joinGamer.getGolds());
                            } else {
                                om.putByte((byte) 0);
                        ...
                        log.info("overGameSendMessage|end|" + (System.currentTimeMillis() - now));
                    }
                }
            }
            log.debug("广播结束！");
