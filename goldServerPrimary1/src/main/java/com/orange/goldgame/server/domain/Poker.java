...
public class Poker {
   ...
   //洗牌
	...
	public Card getOneCardByNumber(int index){
		...
		int res = 0;
		do {
			res = MAX_CARD_NUM * ResourceCache.r.nextInt(4) + index;//每张牌有四个花色
			card = cards.get(res);
			 i ++;
	...
	public Card getOneCardByColor(int color){
		...
		int res = 0;
		do {
			res = color * MAX_CARD_NUM + ResourceCache.r.nextInt(MAX_CARD_NUM);
			card = cards.get(res);
			i ++;
		} while (card == null || (card.isUsed() && i <= MAX_RAND_NUM));
		if (card != null && card.isUsed()) {
	...
	public Card getOneCardRandom() {
		...
		int res = 0;
		do {
			index = ResourceCache.r.nextInt(cardSize);
...
//	        System.out.println(i+","+type);
	...
	private HandCards getDouble(int res) {
		...
		Card card = null;
		int index = ResourceCache.r.nextInt(MAX_CARD_NUM);
	...
	private HandCards getStraight(){
		List<Card> list = new ArrayList<Card>();
		Card card = null;
		int res = ResourceCache.r.nextInt(MAX_CARD_NUM - 2);
		for (int i = 0; i < number; i++) {
	...
	private HandCards getFlush(){
		int res = ResourceCache.r.nextInt(4);
		List<Card> list = new ArrayList<Card>();
		Card card = null;
		for (int i = 0; i < number; i++) {
	...
	private HandCards getStraightFlush(){
		...
		int index = 0;
		do {
			int res = ResourceCache.r.nextInt(4) * MAX_CARD_NUM + ResourceCache.r.nextInt(MAX_CARD_NUM - 2);
			list.clear();
			for (int i = 0; i < number; i++) {
				card = cards.get(res);
				if (card!= null && !card.isUsed()) {
