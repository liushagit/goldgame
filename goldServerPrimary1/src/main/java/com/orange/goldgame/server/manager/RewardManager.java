...
public class RewardManager {
    ...
    private int gainRewardByAreaIdAndGolds(int areaId,int golds,Player player){
        //获取奖励规则
    	AppConfig config =ResourceCache.getInstance().getAppConfigs().get(Constants.REWARD_RULE);
//        AppConfig config = appConfigAction.findAppConfigByKey(Constants.REWARD_RULE);
        ...
        boolean isReward=isNextRun(rewardRules,areaId,golds);
        
        if(isReward){
        	GetOnce once = GetOnceManager.getPlayerGetOnce(player, Constants.DAILY_EXCHANGE);
        	boolean isLimit = isRewardExchange(player,once); 
        	if(isLimit){
        		return 0;
        	}
        	//遍历每个奖励规则
    ...
    /**该玩家是否符合奖励条件**/
    private boolean isNextRun(String[] rewardRules, int areaId, int golds) {
		for(String rewardRule : rewardRules){
            String[] rewardStr = rewardRule.split("_");
            String rewardAreaIdStr = rewardStr[0];
            String rewardNeedGoldsStr = rewardStr[1];
            if(rewardAreaIdStr.equals(areaId+"")){
            	if(golds>=Integer.parseInt(rewardNeedGoldsStr)){
            		return true;          		
            	}
            	return false;
            }
	...
	/**判断该用户兑换券是否达到上限**/
    private boolean isRewardExchange(Player player,GetOnce once){
        int limitAmount = Integer.parseInt(ResourceCache.getInstance().getAppConfigs().get(Constants.EXCHANGLE_LIMIT).getAppValue());
