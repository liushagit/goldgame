...
public class ShopServer extends BaseServer {
	...
	public void requestBuyProp(SocketRequest request, SocketResponse response){
		...
		try{
		    ShopManager.getInstance().buyGoods(player, packagePropsId);
		    ...
		    //添加信息
		    PackageProps prop = BaseEngine.getInstace().getPropActionIpml().queryPackagePropsById(packagePropsId);
		    PropsConfig pConfig = ResourceCache.getInstance().getPropsConfigs().get(prop.getPropsId());
		    
		    PlayerInfoCenterManager.getInstance()
		        .addPlayerInfoCenter(player.getId(),true,Constants.BUY_GOODS,"","您成功购买了"+pConfig.getName()+"*"+prop.getPropsNumber()+"!");
	...
	public void comfirmExcchangeGoods(SocketRequest request, SocketResponse response) throws JuiceException {
		...
		Player player = PlayerService.getPlayer(playerId, request.getSession());
		try{
		    ShopManager.getInstance().changeGoods(player, goodsId, cellphone);
		    OutputMessage om = new OutputMessage();
		    PlayerProps pp = PropCacheAction.getplayerPropByPropid(player, PropCacheAction.exchange_prop);
		    int exchangeNum = (pp == null ? 0 : pp.getNumber());
		    ...
		    String msg2 = "您消耗了"+pp.getNumber()+"兑换券，兑换了"+config.getGoodsName()+"，请及时联系客服！";
