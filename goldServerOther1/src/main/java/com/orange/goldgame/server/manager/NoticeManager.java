...
public class NoticeManager {
    ...
    private final String notice_key = "sys_msg";
    ...
    public void addSystemNotice(){
    	 ...
    	 for(NoticeConfig notice : ResourceCache.getInstance().getNoticeConfigs().values()){
    		 sb.append(notice.getDestance()).append("#");
         }
    	 MemcachedResource.save(notice_key, sb.toString(), 60 * 60);
...
//        pushMessage(systemPlayer,message);
    ...
    public void sendPlayerMessage(Player player ,NoticeMessage message) throws GoldException{
        ...
        if(player.getStatus() == Constants.PLAYER_STATUS_BAN){
            return ;
        }
        
        String msg = message.getMessage();
        if (DirtyWordsUtil.existsWords(msg) || DirtyWordsUtil.existsWords(message.getNickName())) {
			return;
		}
        //扣金币
