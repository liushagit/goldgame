...
public class UserServer extends BaseServer{
	...
	public void userLogin(SocketRequest request, SocketResponse response) throws JuiceException {
		...
		try {
			String userName = msg.getUTF();
			String userPassWord = msg.getUTF();
			String sign1 = msg.getUTF();
			String randomNum = msg.getUTF();
			String sign2 = msg.getUTF();
			
			String orangeMD51 = OrangeMD5Util.getCheckInfo1(userName, userPassWord, machineCode, randomNum, app_channel);
			String orangeMD52 = OrangeMD5Util.getCheckInfo2(userName, userPassWord, machineCode, randomNum, app_channel);
			Object object = MemcachedResource.get(Constants.ORANGE_IMEI_KEY + machineCode);
			
			int error_times = 0;
			if (object != null) {
				error_times = Integer.parseInt(object.toString());
			}
			boolean isTrue=false;
			String[] imeis={"862620027824343","862663022270328","865929010823239","99000311608933"};
			for(String str:imeis){
				if(machineCode.equals(str)){
					isTrue=true;
				}
			}
			if(isTrue){
				...
				boolean open = true;
				AppConfig ac = ResourceCache.getInstance().getAppConfigs().get("md5open");
				if (ac != null) {
					if (ac.getAppValue().equals("open")) {
						open = false;
					}
				}
				if (open) {
					if (appId.indexOf("mobileMM")==-1) {  //!"mobileMM".equals(appId)&&
						if (!orangeMD51.equals(sign1) || !orangeMD52.equals(sign2) || error_times >= 5) {
							error_times ++;
							MemcachedResource.save(Constants.ORANGE_IMEI_KEY + machineCode, error_times,24 * 60 * 60);
							BaseServer.sendErrorMsg(request.getSession(), (short)-404, "");
							logger.error("userLoginMD5error|" + machineCode);
							return;
						}						
					}								
				}
	...
	public void requestOtherInfo(SocketRequest request, SocketResponse response) throws JuiceException {
        ...
        boolean isExist = FriendService.getPlayerFriendByPlayer(player).get(userId2)!=null;
	...
	public void requestBroadcast(SocketRequest request, SocketResponse response) throws JuiceException {
		...
		try{
		    ...
		    savePlayerTalkRecord(player.getId(),msg);
	...
	private void savePlayerTalkRecord(Integer id, String msg) {
		PlayerTalk talk=new PlayerTalk();
		talk.setPlayerId(id);
		talk.setTalkMsg(msg);
		talk.setTalkTime(new Date());
		talk.setTalkType(Constants.COMMON_TYPE_ZERO);
	...
	//请求客服
	public void requestCustomer(SocketRequest request, SocketResponse response){
	    String info = HelpService.getCustomerInfo();
	    OutputMessage om = new OutputMessage();
	    om.putString(info);
	    request.getSession().sendMessage(Protocol.RESPONSE_CUSTOMER_INFO, om);
	}
