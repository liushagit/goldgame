...
public class SessionManger {
	...
	public void remove(int player_id) {
		...
		if(player!=null){
		    player.setIslogin(0);
//		    int online = (int)(System.currentTimeMillis()-player.getLoginTime())/(1000*60);
//		    player.setOnlineTime(online);
	...
	public void scan(List<Integer> player_ids){
		for (Integer player_id : player_ids) {
			remove(player_id);
			MemcachedResource.save(Constants.PLAYER_STATUS_KEY + player_id, Constants.PLAYER_STATUS_OFFLINE, 10);
