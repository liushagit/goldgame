...
public class TaskManager {
     ...
     * 是否已完成
     */
    public boolean isComplex(int playerId ,Task task ){
        TaskConfig taskConfig = ResourceCache.getInstance().getTaskConfigMaps().get(task.getTaskid());
...
//                taskAction.update(task);
    ...
    public void driveTaskRate(Player player,int iswin) {
        ...
        List<TaskConfig> taskConfigList = new ArrayList<TaskConfig>();
        //推进系统任务
        taskConfigList = ResourceCache.getInstance().getTaskConfigs().get(TaskType.RACE_TATOL_TYPE);
        driveTaskList(player,taskConfigList);
        //推进每日任务
        taskConfigList = ResourceCache.getInstance().getTaskConfigs().get(TaskType.RACE_TATOL_TYPE_DAILY);
        driveTaskList(player,taskConfigList);
        
        if(iswin==1){
            //推进系统任务
            taskConfigList = ResourceCache.getInstance().getTaskConfigs().get(TaskType.RACE_TYPE);
            driveTaskList(player,taskConfigList);
            //推进每日任务
            taskConfigList = ResourceCache.getInstance().getTaskConfigs().get(TaskType.RACE_TYPE_DAILY);
            driveTaskList(player,taskConfigList);
        }
    }
    ...
    private void driveTaskList(Player player,List<TaskConfig> taskConfigList){
        for(TaskConfig config :taskConfigList){
        	Task task = player.getPlayeTask().get(config.getId());
        	if (task != null) {
        		if(isComplex(player.getId(), task)){
                    continue;
                }
			}
        	task = taskAction.getTaskByPlayerIdAndTaskId(player.getId(), config.getId());
            if(isComplex(player.getId(), task)){
                continue;
            }
	          ...
	          taskAction.update(task);
	          player.getPlayeTask().put(config.getId(), task);
            break;
        }
    }
}
