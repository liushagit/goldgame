...
public class FriendResponse {
	   ...
	   public static void listFriendResponse(SocketResponse response,
	            Player player, List<FriendVo> list,int friendType){
		   ...
		   for(FriendVo fo :list){
			   om.putInt(fo.getFriendId());
			   om.putString(fo.getFriendName());
			   GameSession session = SessionManger.getInstance().getSession(fo.getFriendId());
			   Player friend = null;
			   if (session != null) {
				   friend = PlayerService.getOnLinePlayer(session);
			   }
			   if (friend == null) {
				friend = PlayerService.getSimplePlayerByPlayerId(fo.getFriendId());
			}
			   if(friend != null){
			   ...
			   om.putByte(fo.getIsLover());
			   om.putInt(fo.getQinMiDu());
			   om.putByte(fo.getIsMessage());
		   }
		   om.putByte((byte)friendType);
