...
public class GameStakeThread{
    ...
    private void doTask(){
        try{
            for(GameTable table : tableList){
                ...
                if(isOverGame(table)){
                	removeTable(table);
                	GameTableManager.overGame(table);
                ...
                if(table.isStartGame()
                        && currentTime - table.getLastOperationTime() > MATCH_LIMIT_TIME*1000 && table.isDealCard()){
    ...
    private  boolean isOverGame(GameTable table){
