...
//		return getPlayerAO().getPlayerMsg(account, password);
	...
	@Override
	public List<Player> queryPlayersByCopperRange(int min, int max, int start,
			int end) {
		return DBManager.queryPlayersByCopperRange(min, max, start, end);
	}

	@Override
	public List<Player> queryPlayersByGoldRange(int min, int max, int start,
			int end) {
		return DBManager.queryPlayersByGoldRange(min, max, start, end);
	}

	@Override
	public List<Player> queryPlayersBySex(int sex, int start, int end) {
		return DBManager.queryPlayersBySex(sex, start, end);
	}

	@Override
	public List<Player> queryPlayersByOnline(int isOnline, int start, int end) {
		return DBManager.queryPlayersByOnline(isOnline, start, end);
	}
