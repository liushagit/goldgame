...
import com.orange.goldgame.system.dao.ClientToolNoticeDAO;
import com.orange.goldgame.system.dao.ClientToolNoticeDAOImpl;
...
import com.orange.goldgame.system.dao.PlayerConsumeDAO;
import com.orange.goldgame.system.dao.PlayerConsumeDAOImpl;
...
//			stakeConfig = (StakeConfig) stakeConfigDao.selectByExample(stakeConfigExample);
	...
	public static NoticeConfig insertNoticeConfig(NoticeConfig config) {
		NoticeConfigDAO dao = (NoticeConfigDAO)getDataDao(NoticeConfigDAOImpl.class);
		try {
			int index = dao.insert(config);
			config.setId(index);
			return config;
		} catch (SQLException e) {
			log.error("添加NoticeConfig数据出错", e);
		}
		return null;
	}
	
	public static void updateNoticeConfig(NoticeConfig config) {
		NoticeConfigDAO dao = (NoticeConfigDAO)getDataDao(NoticeConfigDAOImpl.class);
		try {
			dao.updateByPrimaryKey(config);
		} catch (SQLException e) {
			log.error("更新NoticeConfig数据出错", e);
		}
	}
	
	public static void deleteNoticeConfig(int noticeConfigId) {
		NoticeConfigDAO dao = (NoticeConfigDAO)getDataDao(NoticeConfigDAOImpl.class);
		try {
			dao.deleteByPrimaryKey(noticeConfigId);
		} catch (SQLException e) {
			log.error("删除NoticeConfig数据出错", e);
		}
	}
	
	public static NoticeConfig queryNoticeConfig(int noticeConfigId) {
		NoticeConfig config = null;
		NoticeConfigDAO dao = (NoticeConfigDAO)getDataDao(NoticeConfigDAOImpl.class);
		try {
			config = dao.selectByPrimaryKey(noticeConfigId);
		} catch (SQLException e) {
			log.error("查询NoticeConfig数据出错", e);
		}
		return config;
	}
...
//			playerPropsDao.updateByPrimaryKey(pp);
	...
	@SuppressWarnings("unchecked")
	public static int inserFriend(PlayerFriend pf) {
		PlayerFriendDAO dao = (PlayerFriendDAO)getDao(PlayerFriendDAOImpl.class);
		try {
	...
	//帮助类型
	@SuppressWarnings("unchecked")
	public static List<HelpType> getHelpType(){
		List<HelpType> helps = null;
		HelpType help=null;
		HelpTypeDAO dao=(HelpTypeDAO) getDataDao(HelpTypeDAOImpl.class);
		try {
			helps =dao.selectByExample(null);
		} catch (SQLException e) {
			helps = new ArrayList<HelpType>();
		}
		return helps;
	}
	...
	@SuppressWarnings("unchecked")
	public static List<PlayerPay> queryPayByRange(int start , int end){
		List<PlayerPay> list=null;
		PlayerPayDAO dao  = (PlayerPayDAO)getDao(PlayerPayDAOImpl.class);
		PlayerPayExample example=new PlayerPayExample();
		example.setOrderByClause("limit " + start+","+end);
		try {
			list=dao.selectByExample(example);
		} catch (SQLException e) {
			list=new ArrayList<PlayerPay>();
			log.error("query pay infoa by range have exception", e);
		}
		return list;
	}
	...
	@SuppressWarnings("unchecked")
	public static List<Player> queryPlayersByCopperRange(int min, int max, int start,
			int end){
		List<Player> list=null;
		PlayerDAO playerDao = (PlayerDAO) getDao(PlayerDAOImpl.class);
		PlayerExample example = new PlayerExample();
		example.createCriteria().andStatusNotEqualTo(1);
		example.createCriteria().andCopperBetween(min, max);
		example.setOrderByClause("limit "+start+","+end);
		try {
			list = playerDao.selectByExample(example);	
			...
			list=new ArrayList<Player>();
			log.error("通过指定玩家的金币范围查询玩家数据出错",e);
		}	
		return list;
	}
	...
	@SuppressWarnings("unchecked")
	public static List<Player> queryPlayersByGoldRange(int min, int max, int start,
			int end){
		List<Player> list=null;
		PlayerDAO playerDao = (PlayerDAO) getDao(PlayerDAOImpl.class);
		PlayerExample example = new PlayerExample();
		example.createCriteria().andStatusNotEqualTo(1);
		example.createCriteria().andGoldsBetween(min, max);
		example.setOrderByClause("limit "+start+","+end);
		try {
			list = playerDao.selectByExample(example);	
			...
			list=new ArrayList<Player>();
			log.error("通过指定玩家的宝石范围查询玩家数据出错",e);
		}	
		return list;
	}
	...
	@SuppressWarnings("unchecked")
	public static List<Player> queryPlayersBySex(int sex, int start, int end){
		List<Player> list=null;
		PlayerDAO playerDao = (PlayerDAO) getDao(PlayerDAOImpl.class);
		PlayerExample example = new PlayerExample();
		example.createCriteria().andStatusNotEqualTo(1);
		example.createCriteria().andSexEqualTo(sex);
		example.setOrderByClause("limit "+start+","+end);
		try {
			list = playerDao.selectByExample(example);	
	...
	@SuppressWarnings("unchecked")
	public static List<Player> queryPlayersByOnline(int isOnline, int start, int end){
		List<Player> list=null;
		PlayerDAO playerDao = (PlayerDAO) getDao(PlayerDAOImpl.class);
		PlayerExample example = new PlayerExample();
		example.createCriteria().andStatusNotEqualTo(1);
		example.createCriteria().andIsloginEqualTo(isOnline);
		example.setOrderByClause("limit "+start+","+end);
		try {
			list = playerDao.selectByExample(example);	
			...
			list=new ArrayList<Player>();
			log.error("通过判断玩家是否在线查询玩家数据出错",e);
		}	
		return list;
	}
...
//			DBService.commit(talk);
	...
	@SuppressWarnings("unchecked")
	public static List<ZfbPayRecord> queryZFBByPlayerId(int playerId){
		List<ZfbPayRecord> list = new ArrayList<ZfbPayRecord>();
		ZfbPayRecordDAO dao=(ZfbPayRecordDAO) getDao(ZfbPayRecordDAOImpl.class);
		ZfbPayRecordExample example = new ZfbPayRecordExample();
		example.createCriteria().andPlayerIdEqualTo(playerId);
		try {
			list = dao.selectByExample(example);
		} catch (SQLException e) {
			log.error("query zfb pay record error!",e);
		}
		return list;
	}
	...
	public static void insertPlayerConsume(PlayerConsume consume){
		PlayerConsumeDAO dao = (PlayerConsumeDAO) getDao(PlayerConsumeDAOImpl.class);
		try {
			dao.insert(consume);
		} catch (Exception e) {
			log.error("insert player consume error!",e);
		}
	}
	
	@SuppressWarnings("unchecked")
	public static List<PlayerConsume> queryPlayerConsumes(int playerId,byte consumeType,Date start,Date end){
		List<PlayerConsume> list = new ArrayList<PlayerConsume>();
		PlayerConsumeDAO dao = (PlayerConsumeDAO) getDao(PlayerConsumeDAOImpl.class);
		PlayerConsumeExample example = new PlayerConsumeExample();
		example.createCriteria().andPlayerIdEqualTo(playerId).andStateEqualTo(consumeType);
		example.createCriteria().andComsumeTimeBetween(start, end);
		try {
			list = dao.selectByExample(example);
		} catch (Exception e) {
			log.error("query player consumes error!",e);
		}
		return list;
	}
	
	@SuppressWarnings("unchecked")
	public static List<ClientToolNotice> queryClientNotices(){
		List<ClientToolNotice> list = new ArrayList<ClientToolNotice>();
		ClientToolNoticeDAO  dao = (ClientToolNoticeDAO) getDao(ClientToolNoticeDAOImpl.class);
		ClientToolNoticeExample example = new ClientToolNoticeExample();
		example.createCriteria().andStatusEqualTo(Constants.COMMON_TYPE_ZERO);
		example.setOrderByClause("start_time asc limit 10");
		try {
			list = dao.selectByExample(null);
		} catch (Exception e) {
			log.error("query player consumes error!",e);
	...
	public static ClientToolNotice insertClientNotice(ClientToolNotice notice){
		ClientToolNoticeDAO  dao = (ClientToolNoticeDAO) getDao(ClientToolNoticeDAOImpl.class);
		try {
			int index = dao.insert(notice);
			notice.setId(index);
			return notice;
		} catch (Exception e) {
			log.error("query player consumes error!",e);
	...
	public static void updateClientNotice(ClientToolNotice notice){
		ClientToolNoticeDAO  dao = (ClientToolNoticeDAO) getDao(ClientToolNoticeDAOImpl.class);
		try {
			dao.updateByPrimaryKey(notice);
		} catch (Exception e) {
			log.error("query player consumes error!",e);
		}
	}
	
	public static void deleteClientNotice(int noticeId){
		ClientToolNoticeDAO  dao = (ClientToolNoticeDAO) getDao(ClientToolNoticeDAOImpl.class);
		try {
			dao.deleteByPrimaryKey(noticeId);
		} catch (Exception e) {
			log.error("query player consumes error!",e);
		}
	}
	
	public static ClientToolNotice queryClientNotice(int noticeId){
		ClientToolNotice notice = null;
		ClientToolNoticeDAO  dao = (ClientToolNoticeDAO) getDao(ClientToolNoticeDAOImpl.class);
		try {
			notice = dao.selectByPrimaryKey(noticeId);
		} catch (Exception e) {
			log.error("query player consumes error!",e);
		}
		return notice;
