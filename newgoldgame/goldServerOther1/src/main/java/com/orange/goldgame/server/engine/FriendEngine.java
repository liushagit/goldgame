...
public class FriendEngine {
    ...
    public static List<FriendVo> getFriendList(Player player){
    	...
    	Map<Integer,PlayerFriend> friendList = FriendService.getPlayerFriendByPlayer(player,true,FriendService.FRIENDTYPE);
    	...
    	for(PlayerFriend pf :friendList.values()){
    		...
    		if(pf.getFriendId() > 0 && SessionManger.getInstance().getSession(pf.getFriendId())==null){
    		    ...
    		    GameRoom room = GameRoomManager.getInstance().getRoomByPlayerId(pf.getFriendId());
    		    if(room != null){
    		    ...
    		    if(table == null){
    		        ...
    		        if(GameTableManager.isMatch(table)){
                        fo.setIsOnline((byte)3);
                    }
                    else if(table != null && table.getGamerSeat(new Gamer(pf.getFriendId()))!=null){
                        fo.setIsOnline((byte)2);
                    }else{
                        fo.setIsOnline((byte)1);
                    }
    		    }
	...
	public static void addFriend(Player player, Player friend) throws GoldException {
        ...
        PlayerFriend temp = FriendService.getPlayerFriendByPlayer(player,true,FriendService.FRIENDTYPE).get(friend.getId());
        if(temp!=null)
            throw new GoldException("好友已存在！");
        int size = FriendService.getPlayerFriendByPlayer(player,true,FriendService.FRIENDTYPE).size();
	...
	public static Byte removeFriend(Player player, int friendId) throws GoldException{
		...
		pf = FriendService.getPlayerFriendByPlayer(player,true,FriendService.FRIENDTYPE).get(friendId);
		if(pf == null){
