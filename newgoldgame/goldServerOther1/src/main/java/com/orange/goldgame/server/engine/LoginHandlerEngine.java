...
public class LoginHandlerEngine {
	...
	private static Logger logger = LoggerFactory.getLogger(LoginHandlerEngine.class);
	public static void loginHandler(SocketRequest request,SocketResponse response,String nickname,
		...
		if(Integer.parseInt(appVersion) >= Integer.parseInt(appInfo.getVersion())){
			logger.info("version success|" + appVersion + "|" + appInfo.getVersion() + "|" + appInfo.getAppId());
			...
			logger.info("version fail|" + appVersion + "|" + appInfo.getVersion() + "|" + appInfo.getAppId());
