...
//	private static final int load_time =30 * 60 * 1000;  //每半个小时更新一次
	private static final int load_time =10 * 1000;  //每半个小时更新一次
	...
	private static final int WEALTH_RANK=0;   //财富排行榜
	private static final int EXCHANGE_RANK=1; //兑换排行榜
	private static final int CHARM_RANK=2;   //魅力排行榜
	private static final int MATCH_RANK=3;   //比赛排行榜
...
//				rankMaps.put(rankTypeId, ranks);
		...
		Map<String, Ranking> rMap = new HashMap<String, Ranking>();
		for (Ranking r : ranks) {
			rMap.put(r.getPlayerId() + "_" + r.getPlayerName(), r);
		}
		ranks.clear();
		ranks.addAll(rMap.values());
	...
	private static void addChacheRankRecords(){
		List<Ranking> list=null;
		for(int index=0;index<5;index++){
			list = BaseEngine.getInstace().getRankingAction().loadAllRankingByRankId(index);
			rankMaps.put(index, list);			
		}
	}
	...
	private static void addPlayerLover(){
		List<PlayerFriend> playerLovers = BaseEngine.getInstace().getFriendActionIpml().getTop();
		Map<String, PlayerFriend> map = new HashMap<String, PlayerFriend>();
		String key = "";
		for (PlayerFriend pf : playerLovers) {
			if (pf.getPlayerId() > pf.getFriendId()) {
				key = pf.getPlayerId() + "_" + pf.getFriendId();
			}else {
				key = pf.getFriendId() + "_" + pf.getPlayerId();
			}
			map.put(key, pf);
		}
		playerLovers.clear();
		playerLovers.addAll(map.values());
		System.out.println("size ======" + playerLovers.size());
		try {
			throw new Exception();
		} catch (Exception e) {
			e.printStackTrace();
		}
		List<PlayerFriend> list;
		if (playerLovers.size() > 6) {
			list = playerLovers.subList(0, 6);
		}else {
			list = playerLovers;
		}
		Collections.sort(list, new Comparator<PlayerFriend>() {
			@Override
			public int compare(PlayerFriend o1, PlayerFriend o2) {
				return o2.getLoverIntimacy() - o1.getLoverIntimacy();
			}
		});
		...
		BaseEngine.getInstace().getRankingAction().deleteForRankType(4);
		int id = 25;
		for (PlayerFriend pl : list) {
			ranking = new Ranking();
			ranking.setPlayerId(pl.getPlayerId());
			player = PlayerService.getSimplePlayerByPlayerId(pl.getPlayerId());
			ranking.setPlayerName(player.getNickname() + " VS " + pl.getFriendName());
			ranking.setRankType(4);
			ranking.setRankValue(pl.getLoverIntimacy());
			ranking.setId(id);
			id ++;
			BaseEngine.getInstace().getRankingAction().insert(ranking);
